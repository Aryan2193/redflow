...
- 06:55 Control room pass 2 (Aryan): page gutters; the question moved into Ringside as the first human message; "Log" is now "Agent actions", wider, grouped by agent with avatar, role, time, a rail, kind pills (search, open, write, hit, heavy, stands, refuted, fixed, still open, blocked, version) and per-agent counters; the composer only steps in (no mode pills), New question lives in the header top right and turns the composer red until sent or cancelled, Wrap it up sits in the same spot while a bout runs. Four agents judged enough; a fifth would add a call and cost for no new role.
- 07:00 Plain labels everywhere (Aryan): feed pills read Read, Searched the web, Opened a page, Wrote, Objected, Objected severe, Fact confirmed, Fact disproved, Could not verify, Fix accepted, Fix rejected, Rejected an objection, New version, Working now; card stamps Objection, Fix proposed, Fix accepted, Rejected, Not fixed, Confirmed, Disproved, Unverified, Changed, Corrected, From the team; rounds First answer, Objections, Fact check, Revision, Ruling. Decision card gets a warm amber wash (red wash when risks stay open).
- 07:15 Add context: new reducer add_context(roomId, text) appends "Name: text" to room.brief (4,000 chars per add, 12,000 total); briefBlock labels it "Context the team added, true for their situation", so every model reads it on every step of every question. Composer has an Add context button left of the prompt that opens a large box; Chat shows a "Context the models read" block at the top. Decision card: answer first, then what changed; soft green wash (red when risks stay open).
- 07:35 Never decide over the team's head (Aryan): at settle time, if any human note is still unread or the room context changed since the lead last read it, the lead takes one more revision pass (chair synthesize) before the ruling and decision. Capped at two extra passes per question and by the call budget; Wrap it up still decides immediately. New question columns extraPasses and contextSeen (additive, defaults). Bindings regenerated, both servers published, client redeployed.
- 07:30 Lead draft timed out twice on Maincloud (web search plus thinking past 80s), so the room fell back to a critic's draft as version one. Fix: web search only on the first attempt (3 results), retry writes from knowledge; lead timeout 115s with web, 85s without; chair timeout 125s (an 86s revision was seen).
- 07:40 Verified the extra pass live (question 27): a note posted mid-revision led to one more revision (extraPasses 1, version 3), the note consumed, feed line "1 note from the team arrived after the last revision, one more pass before deciding". Chat context block now titled "Context". Landing page rebuilt: headline for four models, sub-line, the cast with roles, five rounds, three reasons, watch-the-live-room card, GitHub link.
- 07:55 Submission prep: README rewritten (what happens, why, how it is built, run it yourself, repo map, team, licence), MIT LICENSE added, stale frontend context docs removed, HANDOFF marked, repo description, homepage and topics set. Homepage copy sharpened: problem-first sub-line, cast with what each model does, five rounds, "while it runs you are in the room too" (step in, add context, watch every move), three reasons.
- 07:50 Phone layout: under 768px the room is three tabs (Spotlight or Decision, Actions with a live dot and count, Chat with count), one panel at a time filling the screen and scrolling on its own; the decision tab is selected automatically when a bout settles. Desktop grid unchanged.
- 08:08 IST: phone fixes: composer is one line with Send, Add context moved into the header on phones (state lifted to Room), header is two rows on phones that flatten on desktop via md:contents, tabs follow the fight (spotlight change, human note, decision) with a Live/Paused pill; tapping a tab pauses following for 45s. Landing copy rewritten: hero, example chips that prefill the box, a real bout 12 decision block, versus table, who it is for. Pages redeployed.
